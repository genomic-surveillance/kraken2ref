...
def test_singleton_report():
    ...
    assert 2697049 in singleton_proc.tree_meta_out.keys(), "Wrong taxID appears to be selected"
    assert singleton_proc.tree_meta_out[2697049]["graph_idx"] == 13, "Graph linked to wrong line in file"
    assert singleton_proc.tree_meta_out[2697049]["source_taxid"] == 694009, "Wrong parent reported"
    assert singleton_proc.tree_meta_out[2697049]["path"] == [(13, 'S'), (14, 'S1')], "Wrong path reported"
...
def test_complex_tree_report():
    ...
    assert sorted(complex_tree_report_proc.tree_meta_out.keys()) == [3149250, 3149251, 3149253, 3185065, 3219756, 3219759, 3285787, 3290795, 3369048, 3514037, 3847902], "Wrong taxIDs appear to be selected"
...
def test_l1_tree_report():
    ...
    assert sorted(l1_tree_report_max_proc.tree_meta_out.keys()) == [10519, 28285], "Wrong taxIDs appear to be selected in mode MAX"
    ...
    assert sorted(l1_tree_report_skew_proc.tree_meta_out.keys()) == [10519, 10533, 28285]
