...
def dump_to_file_chunks_exp(sample_id, tax_to_readids_dict, fq1, fq2, outdir, chunk_size=10_000, buffer_size=io.DEFAULT_BUFFER_SIZE):
    ...
    Args:
        ...
        chunk_size (int): Number of records to process in each batch (default is 1000)
    """
    ### NOTE: THIS VERSION GET THE READS ID WHICH SHOULD BE WRITTEN, BEFORE START TO WRITE TO FILES 
...
def dump_fastqs(args):
    ...
    if fq_load_mode == "full":
        dump_to_file_index(sample_id,tax_to_readids_dict,fq1,fq2,outdir,
                        max_threads=max_threads,buffer_size=buffer_size)
    if fq_load_mode == "chunk":
        dump_to_file_chunks(sample_id,tax_to_readids_dict,fq1,fq2,outdir,
