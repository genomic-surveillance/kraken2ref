...
def test_sort_basic(tmp_path):
    ...
    sort_basic_proc.write_output(prefix = tmp_path, suffix="decomposed")

    sort_reads.sort_reads(sample_id="test_sort_basic",
                          kraken_output="tests/test_set/with_fluseg_db/testset_50x.kraken.output",
                          mode="tree",
                          ref_json_file=f"{tmp_path}/test_sort_basic_decomposed.json",
                          outdir=tmp_path,
                          update_output=False,
                          condense=False)

    outdata = json.load(open(f"{tmp_path}/test_sort_basic_tax_to_reads.json", "r"))
    ...
    with open(f"{tmp_path}/test_sort_basic_unwritten_reads.txt", "r") as unwritten:
